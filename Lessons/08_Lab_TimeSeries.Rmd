...
#inner_join more general than colbind, can use if not same dates and rows  
...
Rain_decomp <- stl(rainfall_ts,s.window="periodic") #s.window = seasonal window
plot(Rain_decomp)
...
rain_Components <- 
  mutate(rain_Components,Observed=BR_complete$Rainfall,Date=BR_complete$Date)
temp_Components <- 
  mutate(temp_Components,Observed=BR_complete$Temperature_C,Date=BR_complete$Date)
