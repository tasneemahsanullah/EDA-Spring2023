...
matrix1[ , 3] #
matrix1[c(12, 14)] #
matrix1[c(12:14)] #slice, works on vectors as well
matrix1[2:4, 1:3] #
...
dataframe1$ID #single column from data frame
...
```{r DataExercise-1a}
high.temp <- c(64,71,72,63,51,52,61,64,67,73)
low.temp <- c(42,49,60,46,32,32,41,47,52,54)
```
...
```{r DataExercise-1b}
high.celsius <- (high.temp-32)*(5/9)
low.celsius <- (low.temp-32)*(5/9)
...
```{r DataExercise-1c}
temp.dataframe <- data.frame(high.temp,high.celsius,low.temp,low.celsius)
names(temp.dataframe) <- c("High Temp (F)","High Temp (C)","Low Temp (F)","Low Temp (C)") 
...
summary(temp.dataframe$`High Temp (F)`)
sd(temp.dataframe$`High Temp (F)`)
```
