...
#Create a plot
Plot_text <-
ggplot(data=data.frame(x=0,y=0))+ # if put aes here then sets it globally
  geom_point(aes(x=x,y=y)) +
  geom_text(
    x=0,
    y=0.025,
    label=expression(paste("Mean is ", mu,",", R^{2},'=0.6')) #paste combine strings
    )

#R^{2} is MathJax
#paste 0 has no space
...
factor(PeterPaul.chem.nutrients$month,
       levels=1:12, #levels are set of unique values
       labels=month.abb) #month abbreviation
...
PeterPaul.chem.nutrients %>% 
  ...
  geom_point(size=0.5,alpha=0.5) + #alpha is transparency
    ...
    labs(
    title="Surface Temperatures by Day of Year",
    x="Day of Year",
