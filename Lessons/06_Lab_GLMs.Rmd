...
#Exercise 1:
ozone.density <- 
  ggplot(EPAair, aes(x = Ozone,fill=as.factor(Year))) + #only x because y is automatic
...
ozone.pm2.5 <- lm(EPAair$PM2.5~EPAair$Ozone)
summary(ozone.pm2.5)

ozone.pm2.5 <- lm(data=EPAair,PM2.5~Ozone) #another way
summary(ozone.pm2.5)
...
EPAair.subset <- 
  EPAair %>%
  select(mean_Lat:Ozone) %>% #need continuous variables so filter and remove
  drop_na()
